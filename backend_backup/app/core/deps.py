...
# 데이터베이스 세션 의존성 임포트 (이름 충돌 방지)
# infrastructure/db/session.py 파일에 정의된 get_db 함수를 get_db_session 이라는 이름으로 임포트합니다.
from app.infrastructure.db.session import get_db as get_db_session
...
from app.core.config import settings # 설정 임포트 (JWT 만료 시간 등)
...
# 데이터베이스 세션을 얻는 의존성 함수 (FastAPI Depends에서 사용될 함수)
...
def get_db() -> Generator:
    ...
    요청 처리가 끝나면 세션을 닫습니다.
    """
    # infrastructure/db/session.py 파일에서 임포트한 get_db_session 함수를 호출합니다.
